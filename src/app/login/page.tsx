"use client"  // ← ESSENCIAL: Client Component
...
export default function LoginPage() {
  ...
  async function handleDemoLogin() {
    ...
    if (result?.error) {
      setError("Não foi possível entrar. Verifique se a conta demo existe no banco.")
      setLoading(null)
    }
  }
      ...
      <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-8 space-y-8">
        ...
        <div className="space-y-4">
          ...
          <button
            ...
            className="w-full bg-white border-2 border-gray-300 text-gray-700 p-4 rounded-xl shadow hover:shadow-lg transition-all duration-200 font-medium flex items-center justify-center gap-3 disabled:opacity-60 disabled:cursor-not-allowed"
          >
            {loading === "google" ? (
              ...
              <>
                <svg className="w-5 h-5" viewBox="0 0 24 24">
                  {/* SVG do Google igual ao original */}
        ...
        <div className="text-xs text-gray-500 text-center p-2 bg-gray-50 rounded-lg">
          Conta demo: gabriel@gymcoach.com
        </div>
      </div>
    </div>
  )
